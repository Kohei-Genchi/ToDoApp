...
export default {
     ...
     * @param {Array} tasks - Array of task objects to share
     * @param {string} email - Email of the recipient
     * @param {string} permission - Permission level (view, edit)
     * @param {boolean} slackAuthRequired - Whether Slack authentication is required
     * @param {string} categoryName - Optional custom category name
     * @returns {Promise} - API response
     */
    async shareSpecificTasks(
        tasks,
        email,
        permission,
        ...
        try {
            ...
            // 1. Create a category for these tasks
            const now = new Date();
            const dateStr = `${now.getFullYear()}/${(now.getMonth() + 1).toString().padStart(2, "0")}/${now.getDate().toString().padStart(2, "0")}`;
            const name = categoryName || `共有タスク (${dateStr})`;
            ...
            console.error("Error sharing specific tasks:", error);
            throw error;
        }
    },
};
